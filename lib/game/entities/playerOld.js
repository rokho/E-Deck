...
.defines(function() {
    ...
    EntityPlayer = ig.Entity.extend({
        ...
        /*
        To check if the correct adjacent tile is free to set a Core down in, we can use
        the above functions. I can't figure exactly how they work right now, but it is
        on the TODO list. 

        Note also that cores are picked up automatically. That will have to change.

        We really must figure out some way to stop it from taking an action, such as
        putting down and picking back up, more slowly, even though the problem will
        be fixed by making pick up manual instead of automatic.
        */

        coreInteract: function() {
            //this.parent();
            if(!this.hasFuel && !this.hasCool) {
                console.log("Player does not have a Core. Grabbing");
                //TODO: Code for picking up Cores
